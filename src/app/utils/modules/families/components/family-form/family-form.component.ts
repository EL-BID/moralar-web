...
export class FamilyFormComponent extends FormComponentClass implements OnInit {
  ...
  constructor(
    ...
    this.form = this.formBuilder.group({
      ...
      priorization: [
        this.workFront,
        this.permanentDisabled,
        this.elderlyOverEighty,
        this.yearsInSextyAndSeventyNine,
        this.womanServedByProtectiveMeasure,
        this.femaleBreadWinner,
        this.singleParent,
        this.familyWithMoreThanFivePeople,
        this.childUnderEighteen,
        this.headOfHouseholdWithoutIncome,
        this.benefitOfContinuedProvision,
        this.familyPurse,
        this.involuntaryCohabitation,
        this.familyIncomeOfUpTwoMinimumWages,
      ],
  ...
  ngOnInit(): void {
    if (this.formData) {
      ...
      this.yearsInSextyAndSeventyNine.patchValue({rate: 14, value:  `${this.formData.priorization.yearsInSextyAndSeventyNine.value}`});
      this.womanServedByProtectiveMeasure.patchValue({rate: 4, value:  `${this.formData.priorization.womanServedByProtectiveMeasure.value}`});
      ...
      this.familyWithMoreThanFivePeople.patchValue({rate: 7, value:  `${this.formData.priorization.familyWithMoreThanFivePeople.value}`});
      this.childUnderEighteen.patchValue({rate: 8, value:  `${this.formData.priorization.childUnderEighteen.value}`});
      this.headOfHouseholdWithoutIncome.patchValue({rate: 9, value:  `${this.formData.priorization.headOfHouseholdWithoutIncome.value}`});
      this.benefitOfContinuedProvision.patchValue({rate: 10, value:  `${this.formData.priorization.benefitOfContinuedProvision.value}`});
      this.familyPurse.patchValue({rate: 11, value:  `${this.formData.priorization.familyPurse.value}`});
      this.involuntaryCohabitation.patchValue({rate: 12, value:  `${this.formData.priorization.involuntaryCohabitation.value}`});
      this.familyIncomeOfUpTwoMinimumWages.patchValue({rate: 13, value:  `${this.formData.priorization.familyIncomeOfUpTwoMinimumWages.value}`});
